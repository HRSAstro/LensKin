...
from src.pipelines.reconstruction import interpolate_reconstruction_to_grid
...
def test_finer_grid_sum_scales_with_pixel_area_ratio():
    ...
    This is the factor that the old sum-preserving rescale incorrectly removed.
    Edge fill outside the mesh convex hull makes the ratio slightly below 4.
    """
    width = 5.0
    mesh_n = 20
    dest_n = 40
...
def test_interpolated_sb_intflux_scales_with_area_not_mesh_sum():
    ...
    recon = np.full(mesh.shape[0], brightness)
    dest = Grid3D.uniform(
        n_pixels=dest_n, pixel_scale=width / dest_n, n_channels=n_channels
    )
    sb_map = interpolate_reconstruction_to_grid(recon, mesh, dest.grid_2d)
